...
mu_general_av = theano.shared(mu_general.mean(axis=0))
mu_specific_av = theano.shared(mu_specific.mean(axis=0))
...
with model:
    ...
    mix_component = pm.Bernoulli('mix_component', p=prob_unusual, shape=(numRegions))

    mu_temp = []
    for i in range(numRegions):
        mu_temp.append((1.0 - mix_component[i]) * mu_general_av[i,:] + mix_component[i] * mu_specific_av[i,:])
    mu = T.stack(mu_temp)
