...
contains
    ...
    function optimal_h(simulated_series, expectation, alpha, h_values, threshold)
      implicit none
      real(kind=8), intent(in) :: alpha, threshold
      real(kind=8), intent(in), dimension(:) :: expectation, h_values
      real(kind=8), intent(in), dimension(:, :) :: simulated_series
      real(kind=8), dimension(size(h_values)) :: fp_rate
      integer :: i
      real(kind=8) :: optimal_h

      call false_positive(simulated_series, expectation, alpha, h_values, fp_rate)

      do i=1,size(fp_rate)
         if (fp_rate(i) < threshold) then
            optimal_h = h_values(i)
            exit
         end if
      end do
      
    end function optimal_h
